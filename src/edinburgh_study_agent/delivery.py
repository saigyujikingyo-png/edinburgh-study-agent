...
def export_files(store, item_ids, mode='files', max_megabytes=16):
    ...
    value = {'format_version': '1', 'export_id': export_id, 'mode': mode,
             ...
             'next_step': ('Metadata only; call mode=files or bundle to deliver originals. No host registration occurred.' if mode == 'manifest' else 'Use native host attachment references when available. Otherwise programmatically save resource.blob as original bytes in the RECEIVING host workspace and verify size/SHA256. Use streamed stdin or a binary writer, not a large shell argument; keep binary out of model text. Preserve manifest filenames. Follow the destination tool schema: if it declares a host-local absolute path string, pass that verified receiving-host path through its upload adapter; otherwise use its real file reference. Never pass BlobResourceContents, uoe://, a different computer path or a fabricated reference. Inspect destination before retry; completion requires destination readback.')}
