...
GUIDANCE = {
 "student": {
  ...
  "nmr": "Use study_nmr for raw NMR samples. Ask only for needed non-secret fields and resume request_id. connect creates a protected local panel; passwords never enter chat/tool arguments. NOMAD needs its own account. Legacy groups 3OR/2OR require explicit HTTP consent. Download an observed selection or a unique match; use study_export_files for original bytes. Never assume old data migrated or process spectra automatically. URL elicitation is used where supported; the panel browser must run on the campus-runtime computer, so mobile/remote credential setup is not covered.",
