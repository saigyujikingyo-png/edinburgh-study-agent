...
GUIDANCE = {
 "student": {
  "workflow": ["For connection checks, call study_status once and reply briefly. Do not read host configuration or ask unrelated personal questions.",
    ...
    "Use study_materials(course=...,query=...) to find files; operation=read/download handles a unique observed item directly. Matches can be cached with visible timestamps. A list refresh needs a course. operation=updates checks new/changed metadata across observed courses in batches of up to three; resume next_course_offset and page finished jobs without rescanning. Inline document attachments are indexed on read/list; download of a page expands its observed originals (up to 30 files). Bounded keyword search inspects up to 3 documents; unseen pages are not proof of absence. No manual Save As.",
  ...
  "nmr": "Call study_nmr directly with sample text; reuse the saved source/group. Missing non-secret fields stay in chat or a capable host form. First setup offers a protected password panel, never GUI automation or password tool arguments. Windows can remember the encrypted legacy group and its HTTP permission; do not ask for those again. connect reuses setup; reconnect replaces a panel; forget removes it. resume retains download intent. NOMAD has its own expiring session. Export verified originals with study_export_files. Remote/mobile credential setup and live NOMAD acceptance remain unverified.",
  "login": "Reuse the private campus profile. Only call study_connect_school after a live job reports needs_login or the user explicitly requests login. Campus Password/MFA stay on the school's page. NMR uses its separate protected plugin connection panel.",
