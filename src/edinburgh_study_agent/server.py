...
mcp = PortableFastMCP("UoE Companion",
    ...
    "For schedules use study_timetable directly (including semester or a PDF item_id); materials use study_materials (including embedded Word/PDF originals: use the document item_id to discover or download, and query to filter its attachments), Learn updates/unread counters use study_messages, public dates/events use study_events. These return ready-to-use data with internal caching. Do not inspect host files, install PDF tools, write parsers or build a website for basic queries. "
...
def study_read_resource(item_id: str, refresh: bool = False) -> CallToolResult:
    """Read an observed Learn page/file. Document pages return independently indexed inline attachments with original filenames and child ids, ready for download. Verified local files return immediately by default without a browser; refresh=True obtains current remote contents. Otherwise returns job_id to poll. Does not submit forms. Dates require source interpretation."""
    store().item(item_id)
