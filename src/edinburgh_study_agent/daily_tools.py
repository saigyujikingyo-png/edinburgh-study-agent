...
DAILY = {
    "study_nmr": "List recent NOMAD datasets without a sample number, or find/download your NMR raw data through NOMAD or the legacy teaching archive. Off-campus network failures give FortiClient VPN guidance. Keep leading-zero sample IDs. Missing information returns a resumable request; passwords use the protected connection panel, never chat or browser automation.",
    ...
    "study_materials": "Find/read/download course files, including inline document attachments. A page item_id discovers its originals; download saves them without GUI. Unmatched queries inspect up to 3 document pages and report remaining coverage. For new or changed files use operation=updates: checks one course or all observed courses in batches of up to 3; continue with next_course_offset. Compare metadata only. Page completed updates via study_school_job. List is cached; refresh needs a course. limit 1..30.",
