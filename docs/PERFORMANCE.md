...
The Windows matrix exposed an actionability timeout in an existing lazy-course-row test. Course discovery now reads rows/slot count and instantly reveals the next placeholder in one DOM operation instead of separate locator calls that wait for layout stability. A continuously animated synthetic row verifies this path without screenshots or fixed coordinate clicks. The existing 25-second completeness bound and partial-coverage handling remain.
